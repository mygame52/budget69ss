...
<tr> 
<td width="44%">[ <a href="webboard-admin.php">กลับหน้าหลักเว็บบอร์ด</a> | <a href="new-admin.php">ตั้งกระทู้ใหม่</a> ]</td>
...
<?
	...
	if(file_exists($config[fileQuestion])) {
		...
		$count = 0;  //เป็นตัวเพิ่ม ค่าตำแหน่ง ของ Array
		for ($i=0 ; $i<sizeof($question); $i++) {
			...
			if( ereg("($search)",$split[0]) ) {
				$ArrayQ_Discover[$count] = $question[$i];  //เก็บคำถามที่ค้นพบลง Array
				$count++; 
			} // จบ if

			// ค้นหาจากคำถาม
			if( ereg("($search)",$split[1]) ) {
				$split[1] = eregi_replace($search,"<b> $search </b>",$split[1]);  // ทำตัวหนาให้คำที่ต้องการค้น
				$ArrayQ_Discover[$count] = "$split[0]|X|$split[1]|X|$split[2]|X|$split[3]|X|$split[4]|X|$split[5]|X|$split[6]|X|";  //เก็บคำถามที่ค้นพบลง Array
				$count++; 
			} // จบ if

			// ค้นหาจากชื่อผู้ตั้งคำถาม
		...
		// แสดงจำนวนของกระทู้ ที่ค้นหาได้..
  		echo "<tr>";
		echo "<form method=post action=\"./search-admin.php\" name=\"SearchForm\" onsubmit=\"return check();\">";
		echo "<td width='44%'>ผลจากการค้นหาคำว่า <font color=red><b>$search</b></font> เจอทั้งหมดจำนวน <font color=red><b>$count</b></font> กระทู้</td>\n";
	...
	if($count>=1) {
		...
		for ($i=$start ; $i<$count ; $i++) {
			...
			if($i<$stop){
				...
				echo "<td><a href='./view-admin.php?No=$numQuestion' >$Question</a> $imgOK $flashOK $chknew</td>\n";
		...
		$rt = $count%$listpage;
		$totalpage = ($rt!=0) ? floor($count/$listpage)+1 : floor($count/$listpage);
		...
		echo "<img src='./pic/closefd.gif'> = คำถามที่ยังไม่มีคนตอบ \n";
		echo "<img src='./pic/openfd.gif'> = คำถามที่ถูกตอบแล้ว \n";
		echo "<img src='./pic/hotfd.gif'> = คำถามสุดฮอต \n";
		echo "</td><td align=right>\n";

		echo "<select name=\"menu\" onChange=\"MM_jumpMenu('parent',this,0)\" class=violet>\n";
        echo "<option value=\"./search-admin.php?search=$search&listpage=$listpage\" $selected[0]>เรียงตามหมายเลขคำถาม</option>\n";
        echo "<option value=\"./search-admin.php?sequence=1&search=$search&listpage=$listpage\" $selected[1]>เรียงตามวันที่มีคนตอบ</option>\n";
        echo "<option value=\"./search-admin.php?sequence=2&search=$search&listpage=$listpage\" $selected[2]>เรียงตามจำนวนคนตอบ</option>\n";
        echo "</select> ";
		
		echo "<select name=\"numpagemenu\" onChange=\"MM_jumpMenu('parent',this,0)\" class=violet>\n";
        echo "<option value=\"./search-admin.php?sequence=$sequence&search=$search&listpage=10\" $listpageselected[10]>10 กระทู้/หน้า</option>\n";
        echo "<option value=\"./search-admin.php?sequence=$sequence&search=$search&listpage=15\" $listpageselected[15]>15 กระทู้/หน้า</option>\n";
        echo "<option value=\"./search-admin.php?sequence=$sequence&search=$search&listpage=20\" $listpageselected[20]>20 กระทู้/หน้า</option>\n";
        echo "<option value=\"./search-admin.php?sequence=$sequence&search=$search&listpage=30\" $listpageselected[30]>30 กระทู้/หน้า</option>\n";
        echo "<option value=\"./search-admin.php?sequence=$sequence&search=$search&listpage=50\" $listpageselected[50]>50 กระทู้/หน้า</option>\n";
        echo "</select>\n";
		...
		if($page>1 && $page<=$totalpage) {
			$prevpage = $page-1;
			echo "<a href='search-admin.php?page=$prevpage&sequence=$sequence&search=$search&listpage=$listpage'>หน้าก่อนนี้=$prevpage</a> \n";
		}
		...
		if($page!=$totalpage) {
			$nextpage = $page+1;
			echo " <a href='search-admin.php?page=$nextpage&sequence=$sequence&search=$search&listpage=$listpage'>หน้าถัดไป=$nextpage</a>\n";
		}
		...
		if($c>1) {
			$prevpage = $c-1;
			echo "<a href='search-admin.php?page=$prevpage&sequence=$sequence&search=$search&listpage=$listpage' title='10 หน้าก่อนนี้'><<</a> \n";
		}
		else{
			echo "<<\n";
		}
		...
		for($i=$c; $i<$page ; $i++) {
			if($i>0)
			echo "<a href='search-admin.php?page=$i&sequence=$sequence&search=$search&listpage=$listpage'>$i</a> \n";
		}
		...
		if($c>=0) {
			if(($c+10)<$totalpage){
				$nextpage = $c+10;
				echo "<a href='search-admin.php?page=$nextpage&sequence=$sequence&search=$search&listpage=$listpage' title='10 หน้าถัดไป'>>></a> \n";
			}
			else
				echo ">>\n";
		}
	...
	// ถ้าค้นหาไม่พบ
		...
		echo "<p align=center>ไม่พบข้อมูลที่ต้องการค้นหา<br></p></b>\n";
		echo "</font> <br><br>\n\n";
	}
} 
...
// ถ้าไม่มีไฟล์ หัวข้อคำถาม ให้ทำตามนี้
	...
	echo "</table>"; // ปิดตารางของข้อความ [ ตั้งกระทู้ใหม่ | กลับหน้าแรก ] ที่อยู่ในแทก html
	echo "<br><br><font color=red class=size3><b>\n"; 
	echo "<p align=center>ไม่สามารถค้นหาได้ เนื่องจากยังไม่มีใครตั้งกระทู้<br></p></b>\n";
	echo "</font> <br><br>\n\n";
}

?> 

<center>[ <a href="new-admin.php">ตั้งกระทู้ใหม่</a> ]
<br><br>
<< กรุณาใช้ถ้อยคำที่สุภาพ >><br><br>
</center>
