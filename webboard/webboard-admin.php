...
<!--
<p align="CENTER"><br>
<b><font face="LilyUPC" size="+2" color=#9400d3><?=$config[txtheader]?></font></b><br>  <?=$config[headerdetail]?> 
<br><br>
</p>
-->
...
/*
		...
		echo "<select name=\"menu\" onChange=\"MM_jumpMenu('_parent',this,0)\" class=violet>\n";
        echo "<option value=\"./webboard-admin.php?listpage=$listpage\" $selected[0]>เรียงตามหมายเลขคำถาม</option>\n";
        echo "<option value=\"./webboard-admin.php?sequence=1&listpage=$listpage\" $selected[1]>เรียงตามวันที่มีคนตอบ</option>\n";
        echo "<option value=\"./webboard-admin.php?sequence=2&listpage=$listpage\" $selected[2]>เรียงตามจำนวนคนตอบ</option>\n";
        echo "</select> "; 
		
		echo "<select name=\"numpagemenu\" onChange=\"MM_jumpMenu('parent',this,0)\" class=violet>\n";
        echo "<option value=\"./webboard-admin.php?sequence=$sequence&listpage=10\" $listpageselected[10]>10 กระทู้/หน้า</option>\n";
        echo "<option value=\"./webboard-admin.php?sequence=$sequence&listpage=15\" $listpageselected[15]>15 กระทู้/หน้า</option>\n";
        echo "<option value=\"./webboard-admin.php?sequence=$sequence&listpage=20\" $listpageselected[20]>20 กระทู้/หน้า</option>\n";
        echo "<option value=\"./webboard-admin.php?sequence=$sequence&listpage=30\" $listpageselected[30]>30 กระทู้/หน้า</option>\n";
        echo "<option value=\"./webboard-admin.php?sequence=$sequence&listpage=50\" $listpageselected[50]>50 กระทู้/หน้า</option>\n";
        echo "</select>\n";
		echo "</td></tr>\n\n";
...
*/
		...
		if($page>1 && $page<=$totalpage) {
			$prevpage = $page-1;
			echo "<a href='webboard-admin.php?page=$prevpage&sequence=$sequence&listpage=$listpage'>หน้าก่อนนี้=$prevpage</a> \n";
		}
		...
		if($page!=$totalpage) {
			$nextpage = $page+1;
			echo " <a href='webboard-admin.php?page=$nextpage&sequence=$sequence&listpage=$listpage'>หน้าถัดไป=$nextpage</a>\n";
		}
		...
		if($c>1) {
			$prevpage = $c-1;
			echo "<a href='webboard-admin.php?page=$prevpage&sequence=$sequence&listpage=$listpage' title='10 หน้าก่อนนี้'><<</a> \n";
		}
		else{
			echo "<<\n";
		}
		...
		for($i=$c; $i<$page ; $i++) {
			if($i>0)
			echo "<a href='webboard-admin.php?page=$i&sequence=$sequence&listpage=$listpage'>$i</a> \n";
		}
		...
		if($c>=0) {
			if(($c+10)<$totalpage){
				$nextpage = $c+10;
				echo "<a href='webboard-admin.php?page=$nextpage&sequence=$sequence&listpage=$listpage' title='10 หน้าถัดไป'>>></a> \n";
			}
			else
				echo ">>\n";
		}
