...
{
	var x=window.confirm("ต้องการลบกระทู้ที่ "+num+" ใช่หรือไม่");
	return (x);
}
...
<?
	...
	$_SESSION["sesuser_wb"] = $user_wb;
	$_SESSION["sespasswd_wb"] = $passwd_wb;
...
<br><br><font color=green class=size3><b>สำหรับเว็บมาสเตอร์ (ลบคำถาม - คำตอบ)</b></font>
</p>

<table width="98%" border="0" align="center" cellpadding="2" cellspacing="0">
<tr> 
<td width="44%">[ <a href="./admin.php">กลับหน้าหลัก Admin</a> | <a href="./logout.php">Logout</a> ]</td>
</tr>
...
<?
	...
	if(file_exists($config[fileQuestion])) {
		...
		// แสดงจำนวนของกระทู้ทั้งหมด และที่เป็นตารางเพราะสืบเนื่องมาจากตารางก่อนหน้านี้ 
  		echo "<tr>";
   		echo "<td width='44%'><font color=green>จํานวนกระทู้ทั้งหมด<b><font color=red> $All_Q </font></b>กระทู้</font></td>";
  		echo "</tr>";
		echo "</table>";
		...
		echo "<td align=center width=47%><font color=#FFFFFF><b>กระทู้ / Topic</b></font></td>\n";
		echo "<td align=center width=27%><font color=#FFFFFF><b>ผู้ตั้งกระทู้</b> [วันที่ถาม]</font></td>\n";
		echo "<td align=center width=7%><font color=#FFFFFF><b>คำตอบ</b></font></td>\n";
		echo "<td align=center width=10%><font color=#FFFFFF><b>ลบกระทู้</b></font></td>\n";
  		echo "</tr>\n\n";
		...
		for ($i=$start ; $i<$All_Q ; $i++) {
			...
			if($i<$stop){
				...
				echo "<td><a href='./admindel_reply.php?No=$numQuestion'> $Question</a> $imgOK  $flashOK $chknew</td>\n";
				echo "<td><font color='#666666'><b> $Name </b>[$Date]</font></td>\n";
				echo "<td align='center'><font  color='#666666'>$countR</font></td>\n";
				echo "<td align=center><a href=\"./edit.php?select=0&admin_Q=$numQuestion\" onClick='return Q_confirm(\"$No\");'>\n";
				echo "<img src=\"./pic/delete.gif\" border=0 alt=\"ลบคำถามหมายเลข $No\"></a></td>\n";
				echo "</tr>\n\n";
			} // end if
        ...
        echo "<option value=\"./admin.php?listpage=$listpage\" $selected[0]>เรียงตามหมายเลขคำถาม</option>\n";
        echo "<option value=\"./admin.php?sequence=1&listpage=$listpage\" $selected[1]>เรียงตามวันที่มีคนตอบ</option>\n";
        echo "<option value=\"./admin.php?sequence=2&listpage=$listpage\" $selected[2]>เรียงตามจำนวนคนตอบ</option>\n";
        echo "</select> "; 
		
		echo "<select name=\"numpagemenu\" onChange=\"MM_jumpMenu('parent',this,0)\" class=violet>\n";
        echo "<option value=\"./admin.php?sequence=$sequence&listpage=10\" $listpageselected[10]>10 กระทู้/หน้า</option>\n";
        echo "<option value=\"./admin.php?sequence=$sequence&listpage=15\" $listpageselected[15]>15 กระทู้/หน้า</option>\n";
        echo "<option value=\"./admin.php?sequence=$sequence&listpage=20\" $listpageselected[20]>20 กระทู้/หน้า</option>\n";
        echo "<option value=\"./admin.php?sequence=$sequence&listpage=30\" $listpageselected[30]>30 กระทู้/หน้า</option>\n";
        echo "<option value=\"./admin.php?sequence=$sequence&listpage=50\" $listpageselected[50]>50 กระทู้/หน้า</option>\n";
        echo "</select>\n";
		...
		if($page>1 && $page<=$totalpage) {
			$prevpage = $page-1;
			echo "<a href='admin.php?page=$prevpage&sequence=$sequence&listpage=$listpage'>หน้าก่อนนี้=$prevpage</a> \n";
		}
		...
		if($page!=$totalpage) {
			$nextpage = $page+1;
			echo " <a href='admin.php?page=$nextpage&sequence=$sequence&listpage=$listpage'>หน้าถัดไป=$nextpage</a>\n";
		}
		...
		if($c>1) {
			$prevpage = $c-1;
			echo "<a href='admin.php?page=$prevpage&sequence=$sequence&listpage=$listpage' title='10 หน้าก่อนนี้'><<</a> \n";
		}
		else{
			echo "<<\n";
		}
		...
		for($i=$c; $i<$page ; $i++) {
			if($i>0)
			echo "<a href='admin.php?page=$i&sequence=$sequence&listpage=$listpage'>$i</a> \n";
		}
		...
		if($c>=0) {
			if(($c+10)<$totalpage){
				$nextpage = $c+10;
				echo "<a href='admin.php?page=$nextpage&sequence=$sequence&listpage=$listpage' title='10 หน้าถัดไป'>>></a> \n";
			}
			else
				echo ">>\n";
		}
