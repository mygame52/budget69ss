...
<tr> 
<td width="44%">[ <a href="webboard.php">กลับหน้าหลักเว็บบอร์ด</a> | <a href="new.php">ตั้งกระทู้ใหม่</a> ]</td>
...
<?
	...
	if($count>=1) {
        ...
        echo "<option value=\"./search.php?search=$search&listpage=$listpage\" $selected[0]>เรียงตามหมายเลขคำถาม</option>\n";
        echo "<option value=\"./search.php?sequence=1&search=$search&listpage=$listpage\" $selected[1]>เรียงตามวันที่มีคนตอบ</option>\n";
        echo "<option value=\"./search.php?sequence=2&search=$search&listpage=$listpage\" $selected[2]>เรียงตามจำนวนคนตอบ</option>\n";
        echo "</select> ";
		
		echo "<select name=\"numpagemenu\" onChange=\"MM_jumpMenu('parent',this,0)\" class=violet>\n";
        echo "<option value=\"./search.php?sequence=$sequence&search=$search&listpage=10\" $listpageselected[10]>10 กระทู้/หน้า</option>\n";
        echo "<option value=\"./search.php?sequence=$sequence&search=$search&listpage=15\" $listpageselected[15]>15 กระทู้/หน้า</option>\n";
        echo "<option value=\"./search.php?sequence=$sequence&search=$search&listpage=20\" $listpageselected[20]>20 กระทู้/หน้า</option>\n";
        echo "<option value=\"./search.php?sequence=$sequence&search=$search&listpage=30\" $listpageselected[30]>30 กระทู้/หน้า</option>\n";
        echo "<option value=\"./search.php?sequence=$sequence&search=$search&listpage=50\" $listpageselected[50]>50 กระทู้/หน้า</option>\n";
        echo "</select>\n";
		...
		if($page>1 && $page<=$totalpage) {
			$prevpage = $page-1;
			echo "<a href='search.php?page=$prevpage&sequence=$sequence&search=$search&listpage=$listpage'>หน้าก่อนนี้=$prevpage</a> \n";
		}
		...
		if($page!=$totalpage) {
			$nextpage = $page+1;
			echo " <a href='search.php?page=$nextpage&sequence=$sequence&search=$search&listpage=$listpage'>หน้าถัดไป=$nextpage</a>\n";
		}
		...
		if($c>1) {
			$prevpage = $c-1;
			echo "<a href='search.php?page=$prevpage&sequence=$sequence&search=$search&listpage=$listpage' title='10 หน้าก่อนนี้'><<</a> \n";
		}
		else{
			echo "<<\n";
		}
		...
		for($i=$c; $i<$page ; $i++) {
			if($i>0)
			echo "<a href='search.php?page=$i&sequence=$sequence&search=$search&listpage=$listpage'>$i</a> \n";
		}
		...
		if($c>=0) {
			if(($c+10)<$totalpage){
				$nextpage = $c+10;
				echo "<a href='search.php?page=$nextpage&sequence=$sequence&search=$search&listpage=$listpage' title='10 หน้าถัดไป'>>></a> \n";
			}
			else
				echo ">>\n";
		}
